import {OrganisationSearchHandler} from './../OrganisationSearchHandler';
...
test('should use collection type "organizations"', async () => {
  const handler = new OrganisationSearchHandler();
  const io = new IoMock();
  ...
  expect(readFileMock.mock.calls[0][0]).toBe('organizations');
});
